...
export const SchemaDisplayPath = ({
  ...
  // Highlight path parameters. Rendered as React nodes, not innerHTML, so a
  // path containing markup stays inert text.
  const highlightedPath = path.split(/(\{[^}]+\})/g).map((part, index) =>
    part.startsWith("{") ? (
